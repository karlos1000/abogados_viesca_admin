...
class iCalEasyReader
{
	...
	public function getEventsCols($idCal=""){
		$arrReadCols = array("DTSTART", "DTEND", "SUMMARY", "DESCRIPTION", "LOCATION");
		// $calData = self::load( file_get_contents( 'https://calendar.google.com/calendar/ical/'.$idCal.'/public/basic.ics' ) );
		$calData = $this->load( file_get_contents( 'https://calendar.google.com/calendar/ical/'.$idCal.'/public/basic.ics' ) );
		$arrEvents = (isset($calData["VEVENT"]))?$calData["VEVENT"]:array();
		$arrEventsFinal = array();

		// get array only events
		foreach ($arrEvents as $key=>$elem){
			$arrEventsTmp = array();
			foreach ($arrReadCols as $valSearch) {
				if(isset($elem[$valSearch])){
					$dataTmp = $elem[$valSearch];

					// check is array
					if(is_array($dataTmp)){
						$dataTmp = $dataTmp["value"];
					}

					//check dates
					if($valSearch=="DTSTART" || $valSearch=="DTEND"){
		    			// $dataTmp = date('d/m/Y h:i A', strtotime($dataTmp));
		    			$dataTmp = date('d/m/Y H:i', strtotime($dataTmp));
					}
					$arrEventsTmp[strtolower($valSearch)] = $dataTmp;
					// echo $valSearch.": ".$dataTmp."<br/>";
				}
			}

			if(count($arrEventsTmp)>0){
				$arrEventsFinal[] = (object)$arrEventsTmp;
			}
		}

		return $arrEventsFinal;
	}
}
